...
import { fetchMetricsCapabilities, fetchNodeMetrics } from "@/api/ui"
...
const mockedMetrics = vi.mocked(fetchNodeMetrics)
...
function mountTab() {
  const wrapper = mount(NodeMetricsTab, {
    props: { object: { metadata: { name: "node-1" } } },
